  ...
  return (
    <div className="flex h-full w-full overflow-hidden bg-slate-100">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        {/* Top nav */}
        <header className="bg-white border-b border-slate-200 px-6 py-3 flex items-center shrink-0">
          <div className="flex items-center gap-2 text-sm text-slate-500">
            <span className="font-semibold text-slate-800">DataGrid</span>
            <span className="text-slate-300">/</span>
            <span className="text-blue-600 font-medium">Upload Dataset</span>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto px-8 py-8">
          ...
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-slate-800">Upload a Dataset</h1>
            <p className="text-sm text-slate-500 mt-1">Upload your data to begin automated profiling and AI-powered querying</p>
          </div>
            ...
            <div className="col-span-2 space-y-4">
              <div className="bg-white rounded-2xl border border-slate-200 p-5">
                ...
                <ol className="space-y-3">
                  {[
                    { n: '01', label: 'Profiling',      desc: 'Types, nulls, outliers, skew, and correlations detected automatically' },
                    { n: '02', label: 'AI insights',    desc: 'Plain-English observations on data quality and structure' },
                    { n: '03', label: 'Ask questions',  desc: 'Natural language to SQL to charts, with live interpretation' },
                    { n: '04', label: 'Lab & export',   desc: 'Generate synthetic data, run scenarios, export to CSV or Excel' },
                  ].map(({ n, label, desc }) => (
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                {recent.slice(0, 6).map((t) => (
                  <button
                    ...
                    disabled={!!loadingId}
                    className="group bg-white rounded-xl border border-slate-200 hover:border-blue-300 hover:shadow-md hover:-translate-y-0.5 px-4 py-3 flex items-center gap-3 text-left transition-all duration-200 disabled:opacity-50 disabled:hover:translate-y-0"
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-slate-400">{t.row_count?.toLocaleString()} rows · {t.columns?.length} cols</p>
                    </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                {samples.map((s) => (
                  <button
                    ...
                    disabled={!!loadingId}
                    className="group bg-white rounded-2xl border border-slate-200 hover:border-blue-300 hover:shadow-lg hover:-translate-y-1 p-5 text-left transition-all duration-200 disabled:opacity-50 disabled:hover:translate-y-0"
                  >
